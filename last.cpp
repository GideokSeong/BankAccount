...
int speLetters(char *strPtr)
{
	int occurs = 0;

	while (*strPtr != '\0')
	{
		if (!isdigit(*strPtr) && !isalpha(*strPtr))
			occurs++;
		strPtr++;
	}
...
{
	int numspC,numLetters, numDigits, length;

	length = strlen(custPass);
	
	numLetters = countLetters(custPass);
	numDigits = countDigits(custPass);
	numspC = speLetters(custPass);
	if (numspC ==1 && numLetters == 6 && numDigits == 1 && length ==8)
		return true;
	else
		return false;
}
...
void account::login_user(int acno, char pass)
{
	account ac;
	bool flag = false;
	ifstream inFile;
	...
	while (inFile.read(reinterpret_cast<char *> (&ac), sizeof(account)))
	{
		for (int i = 0; i < 20; i++)
		{
			if (acno == 9999 && pass == ac.password[i])
			{
			...
			else if (acno == ac.retacno() && acno != 9999 && pass == ac.password[i])
			{
				cout << "\n Login sucessful !" << endl;
				flag = true;
				customerMenu();
...
void account::create_account()
{
	...
	account ac;
	ifstream temp;
	temp.open("account.txt", ios::binary);
	...
	cin.ignore();
	cout << "\n\tEnter a password consisting of exactly 6 letters and 1 digits 1 special letter: " << endl;
	...
	while (!testPassWord(password))
	{
		...
		if (testPassWord(password))
			cout << "\n\tPlease wait - your password is being verified" << endl;
		else
		{
			cout << "\n\tInvalid password. For example, !1abcdea is valid"
				<< "\n\twith exactly 6 letters and 1 digits 1 special letter" << endl;
