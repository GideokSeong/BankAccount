...
public:
	...
	void report();
	char rettype();
	void login_user(int,string);
	void adep(int);
	void draw(int);
};

void account::login_user(int acno,string pass)
{
	...
	{
		if (acno == 9999 && password == ac.password)
		{
		...
		else if (acno == ac.retacno() && acno != 9999 && password == ac.password)
		{
			cout << "\n Login sucessful !" << endl;
			flag = true;
			customerMenu();
...
void account::create_account()
{
	...
	cin >> acno;
	string password = getpass("Please enter the password: ", true); system("pause");
	system("CLS");
	cin.ignore();
